...
For a finer granularity we can now change the resolution of the bins and split the probabilities among them.
...
To answer this the expected loss function can be minimized, i.e. we can search the first derivative and set it to zero. 

\begin{align*}
E\left(L\left(X,q\right)\right) &= p\left(1-2q\right)+q^2 \\
\frac{\partial E\left(L\left(X,q\right)\right)}{\partial q} &= -2p + 2q \\
\rightarrow 0 &= -2p + 2q \\
\Leftrightarrow p &= q
\end{align*}
