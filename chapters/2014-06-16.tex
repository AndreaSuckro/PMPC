...
\begin{align*}
p(FA) = \int_{\theta}^{\infty} \varphi \left( x,\mu_n,\sigma^2 \right)dx &= 1 - \Phi\left(\theta,x_n,\sigma^2\right)\\
                                                                       &= 1 - \Phi\left(\theta-\mu_n,0,\sigma^2 \right)\\   
p(H) = \int_{0}^{\infty} \varphi \left( x,\mu_y,\sigma^2 \right)dx &= 1 - \Phi\left(\theta,x_y,\sigma^2\right)\\
                                                                 &= 1 - \Phi\left(\theta-\mu_n,\mu_y-\mu_n,\sigma^2 \right)
\end{align*}
We may also adapt on the variance by dividing through $\sigma^2$, thus arriving on a Gaussian with variance $\sigma^2 = 1$ and mean $\mu = 0$ in statistics this is called standardising or normalising.
\begin{align*}
p(FA) = 1 - \Phi\left(\frac{\theta-\mu_n}{\sigma^2},0,1 \right) &= 1 - \Phi\left(\theta' \right)\\
p(H) = 1 - \Phi\left(\frac{\theta-\mu_n}{\sigma^2},\frac{\mu_y-\mu_n}{\sigma^2},1 \right) &= 1 - \Phi\left(\theta'-d',0,1\right)\\
                                                                                      &= 1 - \Phi\left(\theta'-d'\right) \\
d' &= \frac{\mu_y-\mu_n}{\sigma^2} \\
\end{align*}
